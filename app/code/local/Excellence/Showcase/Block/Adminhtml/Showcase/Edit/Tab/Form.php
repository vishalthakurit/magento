<?php

class Excellence_Showcase_Block_Adminhtml_Showcase_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
  protected function _prepareForm()
  {
      $form = new Varien_Data_Form();
      $this->setForm($form);
      $fieldset = $form->addFieldset('showcase_form', array('legend'=>Mage::helper('showcase')->__('Item information')));
     
      $fieldset->addField('title', 'text', array(
          'label'     => Mage::helper('showcase')->__('Title'),
          'class'     => 'required-entry',
          'required'  => true,
          'name'      => 'title',
      ));

      $fieldset->addField('filename', 'file', array(
          'label'     => Mage::helper('showcase')->__('File'),
          'required'  => false,
          'name'      => 'filename',
	  ));
		
      $fieldset->addField('status', 'select', array(
          'label'     => Mage::helper('showcase')->__('Status'),
          'name'      => 'status',
          'values'    => array(
              array(
                  'value'     => 1,
                  'label'     => Mage::helper('showcase')->__('Enabled'),
              ),

              array(
                  'value'     => 2,
                  'label'     => Mage::helper('showcase')->__('Disabled'),
              ),
          ),
      ));
     
      $fieldset->addField('content', 'editor', array(
          'name'      => 'content',
          'label'     => Mage::helper('showcase')->__('Content'),
          'title'     => Mage::helper('showcase')->__('Content'),
          'style'     => 'width:700px; height:500px;',
          'wysiwyg'   => false,
          'required'  => true,
      ));
     
      if ( Mage::getSingleton('adminhtml/session')->getShowcaseData() )
      {
          $form->setValues(Mage::getSingleton('adminhtml/session')->getShowcaseData());
          Mage::getSingleton('adminhtml/session')->setShowcaseData(null);
      } elseif ( Mage::registry('showcase_data') ) {
          $form->setValues(Mage::registry('showcase_data')->getData());
      }
      return parent::_prepareForm();
  }
}